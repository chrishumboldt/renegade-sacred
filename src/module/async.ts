...
// entirely on public upsert calls, same as sacredMerge/sacredSelect, so
// it doesn't touch the core, it just reacts and writes.
export function sacredAsync<Args extends any[], D>(
  ...
  // Each call gets its own promise settlement either way, and only the
